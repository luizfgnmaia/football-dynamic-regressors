...
# There are matches in 2016 without odds, for example: https://www.flashscore.com.br/jogo/p21rtyI6/#/resumo-de-jogo/resumo-de-jogo
...
p1 = input_brier %>%
  ggplot(aes(x = home_4, y = bet365_home)) +
...
p2 = input_brier %>%
  ggplot(aes(x = draw_4, y = bet365_draw)) +
  geom_point(col = rgb(0, 0, 0, 0.25)) +
  xlim(0, 1) +
  ylim(0, 1) +
  xlab("Model 4") +
...
p3 = input_brier %>%
  ggplot(aes(x = away_4, y = bet365_away)) +
  geom_point(col = rgb(0, 0, 0, 0.25)) +
  xlim(0, 1) +
  ylim(0, 1) +
  xlab("Model 4") +
...
p123 = grid.arrange(p1, p2, p3, nrow = 1)

ggsave("plots/model4_bet365.png", p123, width = 15, height = 5)
